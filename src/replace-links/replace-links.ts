...
// RegEx patterns
const PROTECTED_REGEX = /(```[\s\S]*?```|`[^`]*`|\[\[[^\]]+\]\]|\[[^\]]+\]\([^)]+\)|https?:\/\/[^\s]+)/g;
const DATE_FORMAT_REGEX = /^\d{4}-\d{2}-\d{2}$/;
const MONTH_NOTE_REGEX = /^[0-9]{1,2}$/;
const CJK_REGEX = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]/u;
const CJK_CANDIDATE_REGEX = /^[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}\s\d]+$/u;
const KOREAN_REGEX = /^[\p{Script=Hangul}]+$/u;
const JAPANESE_REGEX = /^[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\s\d]+$/u;
...
const findBestCandidateInSameNamespace = (
	...
	for (const [key, data] of filteredCandidates) {
		...
		for (let idx = 0; idx < Math.min(candidateSegments.length, filePathSegments.length); idx++) {
			...
			if (filePathDir === "" && settings.baseDir) {
				...
				const getRelativeDepth = (k: string): number => {
					if (k.startsWith(basePrefix)) {
						...
						const relativeParts = k.slice(basePrefix.length).split("/");
				...
				if (candidateSegments.length < currentBestSegments.length || 
...
const processStandardText = (
	...
	outer: while (i < text.length) {
		...
		if (lastCandidate) {
			...
			if (settings.ignoreDateFormats && DATE_FORMAT_REGEX.test(candidate)) {
				result += candidate;
			...
			if (candidateData) {
				...
				if (isKorean) {
					const koreanResult = handleKoreanSpecialCases(text, i, candidate, candidateData, settings);
					if (koreanResult) {
				...
				if (!isCjkCandidate) {
					...
					const right = i + candidate.length < text.length ? text[i + candidate.length] : undefined;
				...
				const isJapaneseCandidate = JAPANESE_REGEX.test(candidate) && !KOREAN_REGEX.test(candidate);
				...
				if (settings.namespaceResolution && candidateData.restrictNamespace && 
		...
		if (settings.namespaceResolution) {
			...
			if (fallbackMatch) {
				...
				// Skip date formats
				if (settings.ignoreDateFormats && DATE_FORMAT_REGEX.test(word)) {
					result += word;
					i += word.length;
					continue outer;
				}
				...
				if (candidateList) {
					...
					if (filteredCandidates.length === 1) {
						...
						const bestCandidate = findBestCandidateInSameNamespace(filteredCandidates, filePath, settings);
